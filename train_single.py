...
class OfflineGWDataset(Dataset):
    def __init__(self, noisy_npz_path, clean_npz_path, is_train=False):
        if not os.path.exists(noisy_npz_path) or not os.path.exists(clean_npz_path):
            raise FileNotFoundError(f"Missing dataset files! Expected:\n- {noisy_npz_path}\n- {clean_npz_path}")
           
        print(f"Loading offline data:\n -> Noisy: {noisy_npz_path}\n -> Clean: {clean_npz_path}\n -> Augmentation: {is_train}")

        self.noisy_data = np.load(noisy_npz_path, mmap_mode='r')['strains']
        self.clean_data = np.load(clean_npz_path, mmap_mode='r')['strains']
        self.is_train = is_train

        assert self.noisy_data.shape == self.clean_data.shape, "Shape mismatch between datasets!"
    ...
    def __getitem__(self, idx):
        ...
        noisy_np = np.array(self.noisy_data[idx])
        clean_np = np.array(self.clean_data[idx])
...
def train():
    ...
    print(f"\n--- Starting Training for {cfg_train['epochs']} Epochs ---")
    ...
    for epoch in range(cfg_train['epochs']):
        ...
        for noisy_signals, clean_signals in train_dataloader:
            ...
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=cfg_train['clip_max_norm'])
        ...
        print(f"Epoch [{epoch+1}/{cfg_train['epochs']}] | Train Loss: {avg_train_loss:.6f} | Val Loss: {avg_val_loss:.6f}")
